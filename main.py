...
def showResult():
    if operator=='+':
        ...
        def showError():
            messagebox.showerror(title='Zero Division Error',message='Please, repeat the operation from start')
            global firstNumber,SecondNumber,operator
            firstNumber=SecondNumber=operator=None
            label['text']=''
        showError()
    else:
        label['text']=str(round(int(firstNumber)/int(label['text']),2))
...
label.grid(row=0,column=0,pady=(15,15),columnspan=5)
